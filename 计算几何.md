...
// 计算三角形面积
double getTriangleArea(Point<double> a, Point<double> b, Point<double> c) {
    double area = std::fabs(cross(b - a, c - a)) / 2.0;
    return area;
}

// 计算弧形面积
double getSectorArea(Point<double> a, Point<double> b) {
    double dotProduct = dot(a, b) / (length(a) * length(b));
...
// 计算圆上两点之间弧度之差
double getAngle(Point<double> a, Point<double> b) {
    double dotProduct = dot(a, b) / (length(a) * length(b));
    dotProduct = std::max(-1.0, std::min(1.0, dotProduct));
    double angle = std::acos(dotProduct);
    if (cross(a, b) < 0) {
...
// 计算弓形面积
double getArcuateArea(Point<double a, Point<double> b) {
    double ans = getSectorArea(a, b) - getTriangleArea(a, b, Point(0.0, 0.0));
    if (cross(a, b) < 0) {
        ans += 2.0 * getTriangleArea(a, b, Point(0.0, 0.0));
    }
    return ans;
}
```
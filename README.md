...
$f[u][j]$: 表示在以u为根的子树中选，且先选出u（因为其他节点都依赖u, 这样表示状态会简单一点，可以往分组背包(其实也不完全是分组背包，也就物品组和枚举决策的概念上比较像分组背包)上面套），总体积不超过j的最大价值。纠正：就是分组背包问题，把每个节点想象成一个背包，他的容量是以他为根的子树大小，则因为依赖关系，在以u为根的子树中选，并且先选出u的话，那么就只能在他的各个儿子为根的子树中选择一个节点，因为如果在同一个子树中选择多个节点的话，就会出现依赖冲突。
